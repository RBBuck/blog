<template>
    <n-tabs v-model:value="tabValue" justify-content="start" type="line">
        ...
        <n-tab-pane name="list" tab="文章列表">
            ...
            <n-space>
                <div @click="toPage(pageNum)" v-for="pageNum in  pageInfo.pageCount">
                   <div :style="'color:' + (pageNum == pageInfo.page ? 'blue' : '')">{{ pageNum }} </div><!-- 按钮变色 -->
                </div>
            </n-space>
        ...
        <n-tab-pane name="add" tab="添加文章">
            ...
            <n-form>
                <n-form-item label="标题"><!-- 赋值给中间的函数 -->
                    <n-input v-model:value="addArticle.title" placeholder="请输入标题" />
                </n-form-item>
                <n-form-item label="分类"><!-- 赋值给中间的函数 -->
                    <n-select v-model:value="addArticle.categoryId" :options="categortyOptions" />
                </n-form-item>
               <n-form-item label="内容"> <!-- 赋值给中间的函数 -->
                    <rich-text-editor v-model="addArticle.content"></rich-text-editor>
                </n-form-item>
                <n-form-item label=""><!-- 提交之后addArticle拿到了所有的数据通过146行处理提交给服务端 -->
...
const loadBlogs = async () => {
    ...
    for (let row of temp_rows) {
        row.content += "..."
        let d = new Date(row.create_time)//后端只能传送一个时间戳
        row.create_time = `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日`//显示时间功能
    }

    blogListInfo.value = temp_rows;
    pageInfo.count = res.data.data.count;//分页运算逻辑                                 //取余大于0时增加一页
...
const loadCategorys = async () => {
    ...
    categortyOptions.value = res.data.rows.map((item) => {//把id和name转换成label和value
        return {
            label: item.name,
            value: item.id
        }
...
//提交功能实现原本的数据在constupdataArticlke里当我们点击updata时全部丢给服务端修改成功之后
const update = async () => {
